...
- 最终需求文档按可配置的 APA Creator 友好模板输出，默认模板来自“APA 流程需求文档模板”的空模板结构
- 需求生成提示词模板和最终需求文档模板可分别在桌面端编辑、保存、打开配置文件或恢复默认；模板保存到当前 Windows 用户配置目录
...
最终文档会收敛为清晰的 APA 需求文档模板，默认包含流程概述、输入与输出、按顺序编号的操作步骤、业务规则与约束、异常场景和参考信息。中间推理内容只用于帮助模型理解，不应作为最终交付文档的大段分析章节。
...
5. 可选：在 `提示词模板` 区域调整分析规则和行业偏好；保留 `{{recorded_example}}`、`{{source_materials}}`、`{{extra_instruction}}`、`{{requirement_document_template}}` 占位符可以让程序把录制步骤、资料内容、补充要求和最终文档模板注入到提示词中
6. 可选：在 `需求文档模板` 区域调整最终 Markdown 的章节结构；默认模板不包含示例业务内容，只保留空模板结构
7. 点击 `生成 APA 需求文档`
